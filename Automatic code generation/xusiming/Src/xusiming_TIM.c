...
{
  {
    ...
    /* Output and update for function-call system: '<Root>/Subsystem1' */
    {
      ...
      uint8_T rtb_Sum1;
      xusiming_M->Timing.clockTick3 = xusiming_M->Timing.clockTick0;
...

      {
        ...
        txData[1] = xusiming_B.DataStoreRead3;
      ...
      if (xusiming_DW.DiscreteTimeIntegrator1_SYSTEM_ENABLE != 0) {
        ...
        xusiming_B.DiscreteTimeIntegrator1 = Subsystem1_ELAPS_T
          * xusiming_DW.DiscreteTimeIntegrator1_PREV_U +
          xusiming_DW.DiscreteTimeIntegrator1_DSTATE;
      }

      xusiming_B.DataStoreRead3 = xusiming_DW.execpt;
      xusiming_B.DataTypeConversion = (uint8_T)((160U * xusiming_DW.ki) >> 4);
      xusiming_B.DataTypeConversion1 = (uint8_T)((160U * xusiming_DW.kp) >> 4);
      if (xusiming_DW.DiscreteTimeIntegrator_SYSTEM_ENABLE != 0) {
        DiscreteTimeIntegrator = xusiming_DW.DiscreteTimeIntegrator_DSTATE;
      } else {
        DiscreteTimeIntegrator = (uint8_T)((uint32_T)(uint8_T)
          (Subsystem1_ELAPS_T * xusiming_DW.DiscreteTimeIntegrator_PREV_U) +
          xusiming_DW.DiscreteTimeIntegrator_DSTATE);
      }

      rtb_Sum1 = (uint8_T)(xusiming_DW.execpt -
                           xusiming_B.TmpRTBAtSubsystem2Outport1);
      rtb_Product = (uint16_T)((uint32_T)xusiming_DW.ki * rtb_Sum1);
      rtb_Product1 = (uint16_T)((uint32_T)(uint8_T)(rtb_Sum1 -
        xusiming_DW.Delay1_DSTATE) * xusiming_DW.kp);
      ...
      xusiming_DW.DiscreteTimeIntegrator1_DSTATE =
        xusiming_B.DiscreteTimeIntegrator1;
      xusiming_DW.DiscreteTimeIntegrator1_PREV_U = ((uint32_T)rtb_Product << 16)
        + ((uint32_T)rtb_Product1 << 16);
