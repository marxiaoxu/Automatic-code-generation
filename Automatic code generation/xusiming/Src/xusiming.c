...
{
  {
    ...
    if (HAL_CAN_GetRxMessage(&hcan1, CAN_RX_FIFO0, &rxHeader, &rxData[0])!=
        HAL_OK) {
      xusiming_B.CAN_Read_Data1_o1 = (uint8_t)0;
    } else {
      /* Update Read_Data:output 0: ready. */
      xusiming_B.CAN_Read_Data1_o1 = (uint8_t)1;

      {
        /* Update output 2. */
        xusiming_B.CAN_Read_Data1_o2 = (uint8_t)rxHeader.IDE;

        /* Update output 3. */
        xusiming_B.CAN_Read_Data1_o4 = (uint8_t)rxHeader.RTR;

        /* Update output 4. */
        xusiming_B.CAN_Read_Data1_o5 = (uint16_t)rxHeader.DLC;

        /* Update output 5. */
        memcpy(&xusiming_B.CAN_Read_Data1_o6[0],&rxData[0],(size_t)rxHeader.DLC);
        if (rxHeader.IDE == CAN_ID_STD) {
          xusiming_B.CAN_Read_Data1_o3 = (uint32_t)rxHeader.StdId;
        } else {
          xusiming_B.CAN_Read_Data1_o3 = (uint32_t)rxHeader.ExtId;
        }
      }
    }
  }

  if (xusiming_B.CAN_Read_Data1_o1 == 1) {
    xusiming_DW.execpt = xusiming_B.CAN_Read_Data1_o6[0];
    xusiming_DW.kp = xusiming_B.CAN_Read_Data1_o6[1];
    xusiming_DW.ki = xusiming_B.CAN_Read_Data1_o6[3];
  }
}
