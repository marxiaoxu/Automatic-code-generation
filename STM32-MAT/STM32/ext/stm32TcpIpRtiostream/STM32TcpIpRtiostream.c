...
//RP_MODIF #ifdef _WIN32
  /* WINDOWS */

//RP_MODIF #if defined(_MSC_VER)
 /* temporarily disable warning triggered
  * by windows.h */
 //RP_MODIF #pragma warning(push)
 //RP_MODIF #pragma warning(disable: 4255)
//RP_MODIF #endif

//RP_MODIF #include <windows.h>

//RP_MODIF #if defined(_MSC_VER)
 /* restore warning */
 //RP_MODIF #pragma warning(pop)
//RP_MODIF #endif

//RP_MODIF # ifdef __LCC__
//RP_MODIF # ifndef __LCC64__
//RP_MODIF #   include <winsock2.h>
//RP_MODIF # endif
//RP_MODIF #   include <errno.h>
//RP_MODIF # endif

//RP_MODIF #define RTIOSTREAM_ECONNRESET WSAECONNRESET

//RP_MODIF #elif defined(VXWORKS)
 /*VxWorks headers*/
//RP_MODIF # include <selectLib.h>
//RP_MODIF # include <sockLib.h>
//RP_MODIF # include <inetLib.h>
//RP_MODIF # include <ioLib.h>
//RP_MODIF # include <taskLib.h>
//RP_MODIF # include <netinet/tcp.h> 

//RP_MODIF #define RTIOSTREAM_ECONNRESET ECONNRESET

//RP_MODIF #else
  /* UNIX */
# include <signal.h>
//RP_MODIF # include <sys/time.h>      /* Linux */
//RP_MODIF # include <sys/types.h>     /* Linux */
//RP_MODIF # include <sys/socket.h>
//RP_MODIF # include <netinet/in.h>    /* Linux */
//RP_MODIF # include <netinet/tcp.h>   /* Linux */
//RP_MODIF # include <arpa/inet.h>     /* Linux */
# include <netdb.h>
# include <errno.h>
//# include <fcntl.h>  
//# include <unistd.h>
...
//RP_MODIF #ifdef USE_MEXPRINTF
//RP_MODIF #include "mex.h"
//RP_MODIF #define printf mexPrintf
//RP_MODIF #define SERVER_PORT_PRINTF(FORMAT, ARG1) mexPrintf(FORMAT, ARG1)
//RP_MODIF #else
 ...
 * potential work. 
 */
//RP_MODIF #ifdef VXWORKS
//RP_MODIF # define EXT_BLOCKING (1)  
//RP_MODIF #else
...
//RP_MODIF #ifdef WIN32
  /* WINDOWS */
//RP_MODIF # define close closesocket
//RP_MODIF # define SOCK_ERR SOCKET_ERROR
//RP_MODIF #else
  /* UNIX, VXWORKS */
...
//RP_MODIF #if (defined(_WIN32)) || (defined(VXWORKS))
   /* socklen_t may not be available */
//RP_MODIF    typedef int rtiostream_socklen_t;
//RP_MODIF #else
...
//RP_MODIF #if (!defined(VXWORKS))
//RP_MODIF static SOCKET clientOpenSocket(char * hostName, unsigned int portNum, CommsProtocol protocol, int udpSendBufSize, int udpRecvBufSize);
//RP_MODIF #endif
...
//RP_MODIF #if (!defined(VXWORKS))
//RP_MODIF static unsigned long nameLookup(char * hostName);
//RP_MODIF #endif
...
static int initConnectionData(int connectionID, 
   ...
   if (isVerbose) {
      ...
      }
      {
         ...
         int optionValue;
         lwip_getsockopt(sock, SOL_SOCKET, SO_SNDBUF, (char *) &optionValue, &optionLen);
         printf("Connection id %d, udpSendBufSize: %d\n", connectionID, optionValue);
         lwip_getsockopt(sock, SOL_SOCKET, SO_RCVBUF, (char *) &optionValue, &optionLen);
...
//RP_MODIF    #if defined(_WIN32) && defined(_MSC_VER)
      /*temporarily disable warning C4127 caused by using FD_SET*/
//RP_MODIF       #pragma warning(push)
//RP_MODIF       #pragma warning(disable: 4127)
//RP_MODIF     #endif 
...
//RP_MODIF     #endif
     ...
     */
    pending = lwip_select((int)(sock + 1), &ReadFds, NULL, NULL, tvalPtr);
    if (pending == SOCK_ERR) {
        retVal = RTIOSTREAM_ERROR;
    }
...
static int initialUDPServerRecvfrom(ConnectionData * connection,
   ...
   *clientSALen = sizeof(*clientSA);
   ...
   nRead = lwip_recvfrom(connection->serverData->listenSock, 
    ...
    size_t        *sizeRecvd)
{
    ...
    if (connection->protocol == TCP_PROTOCOL) {
       nRead = lwip_recv(connection->sock, dst, sizeLim, 0U);
       if (nRead == SOCK_ERR) {
          retVal = RTIOSTREAM_ERROR;
       } else {
       ...
       if (udpPacketBuffer->dataAvail == 0) {         
          ...
          resetUDPPacketBuffer(udpPacketBuffer);
          /* read into buffer */
          nRead = lwip_recv(connection->sock, 
                       udpPacketBuffer->dataPtr, 
                       connection->udpData->maxPacketSize, 
                       0U);
...
static SOCKET serverOpenSocket(int port, char * serverInfoFile, CommsProtocol protocol, int udpSendBufSize, int udpRecvBufSize)
{
    ...
    */
    ...
    if (protocol == TCP_PROTOCOL) {
        lFd = lwip_socket(AF_INET, SOCK_STREAM, 0);
    }
    else {
        lFd = lwip_socket(AF_INET, SOCK_DGRAM, 0);
    }
    ...
    if (lFd == INVALID_SOCKET) {
        ...
        */
        option = 1;
        sockStatus = 
            lwip_setsockopt(lFd,SOL_SOCKET,SO_REUSEADDR,(char*)&option,sizeof(option));
        ...
        {     
            if (lFd != INVALID_SOCKET)
            {
                ...
                option = 1;
                sockStatus = 
                    lwip_setsockopt(lFd,IPPROTO_TCP,TCP_NODELAY,(char*)&option,sizeof(option));
                if (sockStatus == SOCK_ERR) { 
        ...
        if (protocol == UDP_PROTOCOL)
        {
            if (lFd != INVALID_SOCKET)
            {
                ...
                * transfer rate */
                sockStatus = 
                    lwip_setsockopt(lFd, SOL_SOCKET, SO_SNDBUF,(char*)&udpSendBufSize, sizeof(int));
                if (sockStatus == SOCK_ERR) { 
            ...
            if (lFd != INVALID_SOCKET)
            {
                ...
                * possibility of buffer overflow */
                sockStatus = 
                    lwip_setsockopt(lFd, SOL_SOCKET, SO_RCVBUF,(char*)&udpRecvBufSize, sizeof(int));
                if (sockStatus == SOCK_ERR) { 
    ...
    if (lFd != INVALID_SOCKET) {
        sockStatus = lwip_bind(lFd, (struct sockaddr *) &serverAddr, sFdAddSize);
        if (sockStatus == SOCK_ERR) {
...
static void serverAcceptSocket(ConnectionData * connection)
{
   ...
   if ( (pending > 0) && (error==RTIOSTREAM_NO_ERROR) ) {
      if (connection->protocol == TCP_PROTOCOL) {
          ...
          */
         cFd = lwip_accept(connection->serverData->listenSock, 
         ...
         if (error == RTIOSTREAM_ERROR) {
            lwip_close(connection->serverData->listenSock);
         ...
         if (error == RTIOSTREAM_ERROR) {
            ...
            if (lwip_connect(connection->serverData->listenSock, 
                        &clientSA, 
...
static SOCKET clientOpenSocket(char * hostName, unsigned int portNum, CommsProtocol protocol, int udpSendBufSize, int udpRecvBufSize) {
    ...
    if (protocol == UDP_PROTOCOL)
    {
        if (errStatus!=RTIOSTREAM_ERROR)
        {
            ...
            // transfer rate 
           int sockStatus = 
                lwip_setsockopt(cSock, SOL_SOCKET, SO_SNDBUF,(char*)&udpSendBufSize, sizeof(int));
           if (sockStatus == SOCK_ERR) { 
              printf("setsocketopt() SO_SNDBUF call failed.\n");
        ...
        if (errStatus!=RTIOSTREAM_ERROR)
        {
            ...
            // possibility of buffer overflow 
           int sockStatus = 
                lwip_setsockopt(cSock, SOL_SOCKET, SO_RCVBUF,(char*)&udpRecvBufSize, sizeof(int));
           if (sockStatus == SOCK_ERR) { 
              printf("setsocketopt() SO_RCVBUF call failed.\n");
              cSock = INVALID_SOCKET;
              errStatus = RTIOSTREAM_ERROR;
...
//RP_MODIF #ifdef VXWORKS /* UDP is not supported on VxWorks */
//RP_MODIF     if (protocol == UDP_PROTOCOL) {
//RP_MODIF        result = RTIOSTREAM_ERROR;
//RP_MODIF        return result;
//RP_MODIF     }
//RP_MODIF #endif

//RP_MODIF #ifdef _WIN32
//RP_MODIF     {
//RP_MODIF         WSADATA data;
//RP_MODIF         if (WSAStartup((MAKEWORD(1,1)), &data)) {
//RP_MODIF             result = RTIOSTREAM_ERROR;
//RP_MODIF             printf("WSAStartup() call failed.\n");
//RP_MODIF         }
//RP_MODIF     }
//RP_MODIF #endif

    if (result != RTIOSTREAM_ERROR) {
//RP_MODIF         if (isClient == 1) {
//RP_MODIF #if (!defined(VXWORKS)) /* Client side connection not supported on VxWorks */
//RP_MODIF            sock = clientOpenSocket(xHostName, xPortNum, protocol,udpSendBufSize,udpRecvBufSize);
//RP_MODIF            if (sock == INVALID_SOCKET) {
//RP_MODIF               result = RTIOSTREAM_ERROR;
//RP_MODIF            }
//RP_MODIF #endif
//RP_MODIF         } else {           
...
//RP_MODIF     if (connection->isServer) {
    if (connection->sock == INVALID_SOCKET) {
        serverAcceptSocket(connection);
    }

    if (connection->sock != INVALID_SOCKET) {
//RP_MODIF #ifndef VXWORKS
...
//RP_MODIF            retVal = socketDataSet(connection, (char *)src, size, sizeSent);
//RP_MODIF #endif
    }
//RP_MODIF     } else { /* Client stream */
//RP_MODIF         retVal = socketDataSet(connection, src, size, sizeSent);
//RP_MODIF     }
...
//RP_MODIF     } else { /* Client stream */
//RP_MODIF         int pending;
//RP_MODIF         if (connection->blockingRecvTimeout != BLOCKING_RECV_TIMEOUT_NEVER) {
           /* only call costly "select" if necessary */
//RP_MODIF            retVal = socketDataPending(connection->sock, 
//RP_MODIF                                       connection,
//RP_MODIF                                       &pending, 
//RP_MODIF                                       connection->blockingRecvTimeout);
//RP_MODIF         }
//RP_MODIF         else {
           /* block in "recv" if necessary */
//RP_MODIF            pending = 1;
//RP_MODIF         }
//RP_MODIF         if (pending) {
//RP_MODIF             retVal = socketDataGet(connection, (char *)dst, size, sizeRecvd);
//RP_MODIF         }
//RP_MODIF     }
...
//RP_MODIF     if (connection->isServer) {
    ...
    connection->serverData->listenSock = INVALID_SOCKET;       
//RP_MODIF     } else {
//RP_MODIF        SOCKET cSock = connection->sock;
//RP_MODIF        close(cSock);
